    ...
    function updateUI() {
        ...
        if (!isYearChosen) {
            ...
            if (pricingDetailsDiv && selectedYear) {
                const pricing = { '1': '700 DA', '2': '1100 DA', '3': '1300 DA', '4': '1500 DA', '5': '1700 DA' };
                const price = pricing[selectedYear] || 'N/A';
                pricingDetailsDiv.innerHTML = `<p><span class="price-label">Activation for Year ${selectedYear}:</span> <span class="price">${price}</span></p>`;
            }
        } else {
            const isJustActivated = document.querySelector('#activation-page.active');
            if (!document.querySelector('.page-section.active') || isJustActivated || document.querySelector('#year-selection-page.active')) {
                 showPage('#home-page');
            }
        }
...
// --- وظائف مساعدة ---
    ...
    function animateCarouselAssembly(year) {
        ...
        subjects.forEach((subject, i) => {
            ...
            const initialTransform = `rotateX(${i * angle}deg) translateZ(${tz}px) translateY(50px) scale(0.8)`;
        ...
        carouselItems.forEach((item, i) => {
            setTimeout(() => {
                const finalTransform = `rotateX(${i * angle}deg) translateZ(${tz}px)`;
